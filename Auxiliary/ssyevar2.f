      SUBROUTINE ssyevar2(m1, AD, IndexD, m2, AOD, IndexODi,
     &                    IndexODj, tol, maxitr, n, maxn, nev,
     &                    maxnev, ncv, maxncv, itrdone,
     &                    matvecmultdone, infoaupd, infoeupd, Evals,
     &                    Evecs)
c********************************************************************
c     The subroutine ssyevar2 computes the nev lowest lying
c     eigenstates of a symmetric nxn matrix of type real.
c     This code is based on sssimp.f from arpack96.tar.gz.
...
c     Matrix-vector multiplication is done by the subroutine smyav
c     defined below. This subroutine makes use of OpenMP.
c
c     Parameters:
c     m1               int, input
c                      length of AD
c     AD(m1)           real, input, unchanged
...
c     Evals(nev)       real, output
c                      the nev lowest lying eigenvalues
c     Evecs(n,nev)     real, output
c                      the nev lowest lying eigenstates
c
c     Caution:
c     The ARPACK subroutine ssaupd has problems with tol!
c
...
c********************************************************************
      ...
      REAL AD(m1), AOD(m2), tol, Evals(nev), Evecs(n,nev)
      LOGICAL select(maxncv), rvec
      CHARACTER bmat*1, which*2
      INTEGER ldv, iparam(11), ipntr(11), ido, lworkl, info, ierr, i
      INTEGER j, nx, ishfts, mode1, nconv
      REAL v(maxn,maxncv), workl(maxncv*(maxncv+8))
      REAL workd(3*maxn), d(maxncv,2), resid(maxn)
      REAL ax(maxn), sigma, zero, snrm2
      ...
      iparam(7)=mode1
 10   continue
       call ssaupd(ido,bmat,n,which,nev,tol,resid,ncv,v,ldv,iparam,
     &             ipntr,workd,workl,lworkl,info)
       if ((ido .eq. -1) .or. (ido .eq. 1)) then
        call smyav(m1,AD,IndexD,m2,AOD,IndexODi,IndexODj,n,
     &            workd(ipntr(1)),workd(ipntr(2)))
      ...
      if (info .lt. 0) then
       ...
       rvec=.true.
       call sseupd(rvec,'All',select,d,v,ldv,sigma,bmat,n,which,nev,
     ...
     &             lworkl,ierr)
       ...
       if (ierr .ne. 0) then
        ...
        nconv=iparam(5)
        do 20 j=1, nconv
         call smyav(m1,AD,IndexD,m2,AOD,IndexODi,IndexODj,n,v(1,j),ax)
 ...
 30     continue
      ...
      SUBROUTINE smyav(m1,AD,IndexD,m2,AOD,IndexODi,IndexODj,n,v,w)
      INTEGER m1, IndexD(m1), m2, IndexODi(m2), IndexODj(m2), n
      REAL AD(m1), AOD(m2), v(n), w(n)
      INTEGER i, j, k
